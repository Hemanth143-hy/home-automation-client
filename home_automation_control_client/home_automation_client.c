...
static void                     hac_interrupt_handler(void* user_data, uint8_t value);
...
void hac_application_init(void) {
...
#if (defined(CYW20719B1) || defined(CYW20721B1) || defined(CYW20735B1))
    wiced_platform_register_button_callback(WICED_PLATFORM_BUTTON_1 , hac_interrupt_handler , NULL , WICED_PLATFORM_BUTTON_BOTH_EDGE);
#else
    wiced_hal_gpio_register_pin_for_interrupt(WICED_PLATFORM_BUTTON_1, hac_interrupt_handler,NULL);
    wiced_hal_gpio_configure_pin(WICED_PLATFORM_BUTTON_1, APP_BUTTON_SETTINGS,APP_BUTTON_DEFAULT_STATE);
#endif
    ...
    if (wiced_init_timer(&app_timer, hac_timeout, 0,WICED_SECONDS_PERIODIC_TIMER) == WICED_SUCCESS) {
...
void hac_interrupt_handler(void* user_data, uint8_t value) {
    ...
    WICED_BT_TRACE("hac_interrupt_handler , app timer : %d\n",hac_app_cb.timeout);
...
#else
    if (wiced_hal_gpio_get_pin_input_status(WICED_GPIO_PIN_BUTTON_1) == WICED_BUTTON_PRESSED_VALUE)
#endif
    {
        WICED_BT_TRACE(" BUTTON_PRESSED\n");
        button_pushed_time = hac_app_cb.timeout;
    } else if (button_pushed_time != 0)
    {
        WICED_BT_TRACE("BUTTON_RELEASED \n");
        if (hac_app_cb.conn_id && (hac_app_cb.home_automation_service_s_handle != 0) && (hac_app_cb.home_automation_service_e_handle != 0))
        {
            if (hac_app_cb.timeout - button_pushed_time == 2)
            {
                WICED_BT_TRACE("the button press time is 2 seconds onn the fan ");
                status = wiced_bt_hac_fan_control(hac_app_cb.conn_id);
                WICED_BT_TRACE(" %d \n", status);
            } else if ((hac_app_cb.timeout - button_pushed_time > 2) && (hac_app_cb.timeout - button_pushed_time <= 4)) {
                WICED_BT_TRACE("between 2 and 4 seconds , fann OFF");
                  status = wiced_bt_hac_fan_off_control(hac_app_cb.conn_id);
                WICED_BT_TRACE(" %d \n", status);
            } else if ((hac_app_cb.timeout - button_pushed_time > 4) &&(hac_app_cb.timeout - button_pushed_time <= 6)) {
                WICED_BT_TRACE("Between 4 and 6 seconds , light On");
                   status = wiced_bt_hac_light_on_control(hac_app_cb.conn_id);
                   WICED_BT_TRACE("%d \n" , status);
            }else if ((hac_app_cb.timeout - button_pushed_time > 6) &&(hac_app_cb.timeout - button_pushed_time <= 8)) {
                WICED_BT_TRACE("more than 6 and less than 8 seconds , light Off");
                   status = wiced_bt_hac_light_off_control(hac_app_cb.conn_id);
                   WICED_BT_TRACE("%d \n" , status);

            }
            else if ((hac_app_cb.timeout - button_pushed_time > 8) && (hac_app_cb.timeout - button_pushed_time <= 10) )
            {
                WICED_BT_TRACE("more than 8 less than 10 seconds ,fan speed changing");
                status = wiced_bt_hac_fan_speed_control(hac_app_cb.conn_id);
                WICED_BT_TRACE("%d \n" , status);
            }
            else if (hac_app_cb.timeout - button_pushed_time > 10)
            {
                WICED_BT_TRACE("more than 10seconds ,  light brightness changing ");
        ...
        } else if (hac_app_cb.timeout - button_pushed_time < 2) {
            WICED_BT_TRACE("Before 2 sec");
            /*device not connected  and no scan in process*/
            if ((hac_app_cb.conn_id == 0)&& (wiced_bt_ble_get_current_scan_state()== BTM_BLE_SCAN_TYPE_NONE)) {
                result = wiced_bt_ble_scan(BTM_BLE_SCAN_TYPE_HIGH_DUTY, WICED_TRUE, hac_scan_result_cback);
                WICED_BT_TRACE("wiced_bt_ble_scan :%d\n", result);
            }
        }
